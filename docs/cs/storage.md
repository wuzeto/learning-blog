...
1. **首次适应（FF，2019）**：从低地址开始找第一个满足的分区

2. **循环首次适应（NF，2019）**：从上一次找到的位置开始

3. **最佳适应（BF，2019）**：找大小最接近的分区

4. **最坏适应（WF，2019）**：找最大的分区
...
* 有效访问时间：$T_e = (1-p) \times T_a + p \times T_f$



**页表基地址寄存器**（2024）：

* PDBR（Page Directory Base Register，2018）
...
1. **最佳置换（OPT，2014）**：淘汰未来最长时间不使用

2. **先进先出（FIFO，2014）**：淘汰最早调入的页

    * **Belady异常**（2014）：增加物理块数可能增加缺页率

3. **最近最少使用（LRU，2014）**：淘汰最近最久未使用的页

    * 维护访问时间栈或计数器

    * 缺页且无空闲页框时，选择访问时间最早的页面淘汰
...
* 工作集$W(t, \Delta)$：时间$t$前$\Delta$时间内访问的页面集合
...
* **固定分配局部置换**（2010）：进程物理块数固定

* **可变分配全局置换**：OS动态调整各进程物理块数
