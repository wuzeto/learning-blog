...
f(x) \leq g(x) \leq h(x)
\\]
且 \\( \lim_{x \to x_0} f(x) = \lim_{x \to x_0} h(x) = L \\)，则
\\[
\lim_{x \to x_0} g(x) = L
\\]
...
**Leibniz 求导法则**  
若 \\( u(x), v(x) \\) 在 \\( x \\) 处 \\( n \\) 阶可导，则
\\[
(u \cdot v)^{(n)} = \sum_{k=0}^n \binom{n}{k} u^{(k)} v^{(n-k)}
\\]

**Rolle 定理**  
若 \\( f(x) \\) 在 \\( [a, b] \\) 上连续，在 \\( (a, b) \\) 内可导，且 \\( f(a) = f(b) \\)，则存在 \\( \xi \in (a, b) \\) 使得
\\[
f'(\xi) = 0
\\]
...
**类型二：\\( y'' = f(x, y') \\)**  
令 \\( p = y' \\)，则 \\( y'' = \frac{dp}{dx} \\)，方程化为：
\\[
\frac{dp}{dx} = f(x, p)
\\]

**类型三：\\( y'' = f(y, y') \\)**  
令 \\( p = y' \\)，则 \\( y'' = p\frac{dp}{dy} \\)，方程化为：
\\[
p\frac{dp}{dy} = f(y, p)
\\]
...
**常数变易法**  
若齐次通解为 \\( y = C_1y_1 + C_2y_2 \\)，则设特解为：
\\[
y^* = v_1(x)y_1 + v_2(x)y_2
\\]
其中 \\( v_1', v_2' \\) 满足：
\\[
\begin{cases}
...
解得：
\\[
v_1' = -\frac{y_2f}{W}, \quad v_2' = \frac{y_1f}{W}
\\]
...
**矩阵指数法**  
对于 \\( \frac{d\mathbf{X}}{dt} = \mathbf{A}\mathbf{X} \\)，通解为：
\\[
...
**特征值法**  
设解为 \\( \mathbf{X} = \mathbf{v}e^{\lambda t} \\)，代入得：
\\[
...
**常数变易法**  
若齐次通解为 \\( \mathbf{X} = \mathbf{\Phi}(t)\mathbf{C} \\)，则设特解为：
\\[
\mathbf{X}^* = \mathbf{\Phi}(t)\mathbf{v}(t)
\\]
...
即：
\\[
\frac{d}{dx}\begin{bmatrix} y_1 \\ y_2 \end{bmatrix} = 
\begin{bmatrix}
0 & 1 \\
-q & -p
\end{bmatrix}
...
**多元Cantor定理**  
设 \\( D \subset \mathbb{R}^n \\) 是有界闭区域，函数列 \\( \\{f_k(\vv{x})\\} \\) 在 \\( D \\) 上满足：
1.  单调性：\\( f_1(\vv{x}) \geq f_2(\vv{x}) \geq \cdots \geq f_k(\vv{x}) \geq \cdots \\)（或单调递减）
2.  连续性：每个 \\( f_k(\vv{x}) \\) 在 \\( D \\) 上连续
3.  收敛性：\\( \lim_{k \to \infty} f_k(\vv{x}) = f(\vv{x}) \\) 在 \\( D \\) 上处处存在
则 \\( f_k(\vv{x}) \\) 在 \\( D \\) 上一致收敛于 \\( f(\vv{x}) \\)。
...
**多元函数极限Cauchy准则**  
设 \\( f: D \subset \mathbb{R}^n \to \mathbb{R}^m \\)，\\( \vv{x}_0 \\) 是 \\( D \\) 的聚点，则极限
\\[
\lim_{\vv{x} \to \vv{x}_0} f(\vv{x}) = \vv{L}
\\]
存在的充要条件是：
\\[
\forall \varepsilon > 0, \exists \delta > 0, \forall \vv{x}, \vv{y} \in \mathring{U}(\vv{x}_0, \delta) \cap D: \|f(\vv{x}) - f(\vv{y})\| < \varepsilon
\\]

**多元函数序列Cauchy准则**  
函数列 \\( \\{f_k(\vv{x})\\} \\) 在 \\( D \\) 上一致收敛的充要条件是：
\\[
\forall \varepsilon > 0, \exists N \in \mathbb{N}, \forall k, l > N, \forall \vv{x} \in D: |f_k(\vv{x}) - f_l(\vv{x})| < \varepsilon
\\]

### 夹逼定理

**多元函数夹逼定理**  
设 \\( f, g, h: D \subset \mathbb{R}^n \to \mathbb{R} \\)，\\( \vv{x}_0 \\) 是 \\( D \\) 的聚点，且在 \\( \vv{x}_0 \\) 的某个去心邻域内满足：
\\[
g(\vv{x}) \leq f(\vv{x}) \leq h(\vv{x})
\\]
如果
\\[
\lim_{\vv{x} \to \vv{x}_0} g(\vv{x}) = \lim_{\vv{x} \to \vv{x}_0} h(\vv{x}) = L
\\]
则
\\[
\lim_{\vv{x} \to \vv{x}_0} f(\vv{x}) = L
\\]

> **示例：**  
> 证明 \\( \lim_{(x,y) \to (0,0)} \frac{x^2y}{x^2 + y^2} = 0 \\)  
> 解：由于 \\( |x| \leq \sqrt{x^2 + y^2} \\)，\\( |y| \leq \sqrt{x^2 + y^2} \\)，所以
> \\[
> 0 \leq \left|\frac{x^2y}{x^2 + y^2}\right| \leq |y| \leq \sqrt{x^2 + y^2}
> \\]
> 由夹逼定理得极限为 0。
...
**多元函数Heine原理**  
设 \\( f: D \subset \mathbb{R}^n \to \mathbb{R}^m \\)，\\( \vv{x}_0 \\) 是 \\( D \\) 的聚点，则
\\[
\lim_{\vv{x} \to \vv{x}_0} f(\vv{x}) = \vv{L}
\\]
的充要条件是：对于任意满足 \\( \vv{x}_k \to \vv{x}_0 \\)，\\( \vv{x}_k \neq \vv{x}_0 \\)，\\( \vv{x}_k \in D \\) 的点列 \\( \\{\vv{x}_k\\} \\)，都有
\\[
\lim_{k \to \infty} f(\vv{x}_k) = \vv{L}
\\]
...
**多元Taylor公式**  
设 \\( f: U \subset \mathbb{R}^n \to \mathbb{R} \\) 在点 \\( \vv{a} \in U \\) 处有 \\( m+1 \\) 阶连续偏导数，则对任意 \\( \vv{h} = (h_1, \dots, h_n) \\) 使得 \\( \vv{a} + \vv{h} \in U \\)，有
\\[
f(\vv{a} + \vv{h}) = \sum_{k=0}^m \frac{1}{k!} \left(\sum_{i=1}^n h_i \frac{\partial}{\partial x_i}\right)^k f(\vv{a}) + R_m(\vv{h})
\\]
其中余项 \\( R_m(\vv{h}) \\) 可表示为：
*   Peano 余项：\\( R_m(\vv{h}) = o(\|\vv{h}\|^m) \\)
*   Lagrange 余项：存在 \\( \theta \in (0,1) \\) 使得
    \\[
    R_m(\vv{h}) = \frac{1}{(m+1)!} \left(\sum_{i=1}^n h_i \frac{\partial}{\partial x_i}\right)^{m+1} f(\vv{a} + \theta\vv{h})
    \\]

**二阶Taylor展开**  
\\[
f(\vv{a} + \vv{h}) = f(\vv{a}) + \sum_{i=1}^n \frac{\partial f}{\partial x_i}(\vv{a})h_i + \frac{1}{2}\sum_{i,j=1}^n \frac{\partial^2 f}{\partial x_i \partial x_j}(\vv{a})h_i h_j + o(\|\vv{h}\|^2)
\\]
或写成矩阵形式：
\\[
f(\vv{a} + \vv{h}) = f(\vv{a}) + \nabla f(\vv{a})^T \vv{h} + \frac{1}{2} \vv{h}^T H_f(\vv{a}) \vv{h} + o(\|\vv{h}\|^2)
\\]
其中 \\( H_f(\vv{a}) \\) 是 Hessian 矩阵。
...
& + \frac{1}{2}[f_{xx}(a,b)h^2 + 2f_{xy}(a,b)hk + f_{yy}(a,b)k^2] + o(h^2 + k^2)
\end{aligned}
\\]
...
1.  \\( F(\vv{x}_0, y_0) = 0 \\)，其中 \\( \vv{x}_0 \in \mathbb{R}^n \\)，\\( y_0 \in \mathbb{R} \\)
2.  在 \\( (\vv{x}_0, y_0) \\) 的某邻域内，\\( F \\) 有连续偏导数
3.  \\( \frac{\partial F}{\partial y}(\vv{x}_0, y_0) \neq 0 \\)
则在 \\( \vv{x}_0 \\) 的某邻域 \\( U \subset \mathbb{R}^n \\) 内存在唯一函数 \\( y = f(\vv{x}) \\) 满足：
1.  \\( f(\vv{x}_0) = y_0 \\)
2.  \\( F(\vv{x}, f(\vv{x})) \equiv 0 \\)
3.  \\( f \\) 在 \\( U \\) 内连续可微，且
    \\[
    \frac{\partial f}{\partial x_i}(\vv{x}) = -\frac{\frac{\partial F}{\partial x_i}(\vv{x}, f(\vv{x}))}{\frac{\partial F}{\partial y}(\vv{x}, f(\vv{x}))}, \quad i = 1, \dots, n
    \\]

**示例**  
方程 \\( F(x,y) = x^2 + y^2 - 1 = 0 \\) 在 \\( (0,1) \\) 附近确定隐函数 \\( y = f(x) \\)，且
\\[
...
1.  \\( F_i(\vv{x}_0, \vv{y}_0) = 0 \\)，其中 \\( \vv{x}_0 \in \mathbb{R}^n \\)，\\( \vv{y}_0 \in \mathbb{R}^m \\)
2.  在 \\( (\vv{x}_0, \vv{y}_0) \\) 的某邻域内，\\( F_i \\) 有连续偏导数
3.  Jacobi 行列式在 \\( (\vv{x}_0, \vv{y}_0) \\) 处不为零：
    \\[
    \frac{\partial(F_1, \dots, F_m)}{\partial(y_1, \dots, y_m)}(\vv{x}_0, \vv{y}_0) \neq 0
    \\]
则在 \\( \vv{x}_0 \\) 的某邻域 \\( U \subset \mathbb{R}^n \\) 内存在唯一函数组 \\( \vv{y} = \vv{f}(\vv{x}) = (f_1(\vv{x}), \dots, f_m(\vv{x})) \\) 满足：
1.  \\( \vv{f}(\vv{x}_0) = \vv{y}_0 \\)
2.  \\( F_i(\vv{x}, \vv{f}(\vv{x})) \equiv 0, \quad i = 1, \dots, m \\)
3.  \\( \vv{f} \\) 在 \\( U \\) 内连续可微

**隐函数组微分公式**  
在定理条件下，有
\\[
\frac{\partial \vv{f}}{\partial \vv{x}} = -\left(\frac{\partial \vv{F}}{\partial \vv{y}}\right)^{-1} \frac{\partial \vv{F}}{\partial \vv{x}}
\\]
即
\\[
\begin{bmatrix}
...
**反函数组定理**  
设函数组 \\( \vv{y} = \vv{f}(\vv{x}): U \subset \mathbb{R}^n \to \mathbb{R}^n \\) 满足：
1.  \\( \vv{f} \\) 在 \\( \vv{x}_0 \\) 处可微
2.  Jacobi 行列式在 \\( \vv{x}_0 \\) 处不为零：
    \\[
    J = \frac{\partial(f_1, \dots, f_n)}{\partial(x_1, \dots, x_n)}(\vv{x}_0) \neq 0
    \\]
则在 \\( \vv{y}_0 = \vv{f}(\vv{x}_0) \\) 的某邻域内存在唯一反函数组 \\( \vv{x} = \vv{g}(\vv{y}) \\)，且
\\[
\frac{\partial \vv{g}}{\partial \vv{y}} = \left(\frac{\partial \vv{f}}{\partial \vv{x}}\right)^{-1}
\\]
...
\begin{cases}
\min f(\vv{x}) \\
g_i(\vv{x}) = 0, \quad i = 1, \dots, m
\end{cases}
\\]
若 \\( \vv{x}^* \\) 是极值点，且 \\( \nabla g_1(\vv{x}^*), \dots, \nabla g_m(\vv{x}^*) \\) 线性无关，则存在 Lagrange 乘数 \\( \lambda_1, \dots, \lambda_m \\) 使得
\\[
\nabla f(\vv{x}^*) = \sum_{i=1}^m \lambda_i \nabla g_i(\vv{x}^*)
\\]

### 曲面的切平面与法线

**隐式曲面切平面**  
设曲面由 \\( F(x,y,z) = 0 \\) 确定，\\( P(x_0,y_0,z_0) \\) 是曲面上一点，且 \\( \nabla F(P) \neq \vv{0} \\)，则曲面在 \\( P \\) 点的切平面方程为：
\\[
...
**参数曲面切平面**  
设曲面由 \\( \vv{r}(u,v) = (x(u,v), y(u,v), z(u,v)) \\) 给出，则切平面由向量
\\[
\frac{\partial \vv{r}}{\partial u} \times \frac{\partial \vv{r}}{\partial v}
\\]
决定。
...
*   **一阶展开**：\\( f(\vv{a} + \vv{h}) = f(\vv{a}) + \nabla f(\vv{a}) \cdot \vv{h} + o(\|\vv{h}\|) \\)
*   **二阶展开**：\\( f(\vv{a} + \vv{h}) = f(\vv{a}) + \nabla f(\vv{a})^T \vv{h} + \frac{1}{2} \vv{h}^T H_f(\vv{a}) \vv{h} + o(\|\vv{h}\|^2) \\)
*   **二元函数**：
    \\[
    \begin{aligned}
    f(a+h,b+k) = & f(a,b) + f_x h + f_y k \\
    & + \frac{1}{2}(f_{xx}h^2 + 2f_{xy}hk + f_{yy}k^2) + \cdots
    \end{aligned}
    \\]
...
*   **Euler 方程** \\( x^2y'' + pxy' + qy = 0 \\)：
    \\[
    \begin{aligned}
    \Delta > 0 &: y = C_1x^{r_1} + C_2x^{r_2} \\
    \Delta = 0 &: y = (C_1 + C_2\ln x)x^{r} \\
    \Delta < 0 &: y = x^{\alpha}[C_1\cos(\beta\ln x) + C_2\sin(\beta\ln x)]
    \end{aligned}
    \\]
