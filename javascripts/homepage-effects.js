...
document.addEventListener('DOMContentLoaded', function() {
      ...
      function animate() {
        ...
        for (let x = 0; x < gridWidth; x++) {
          for (let y = 0; y < gridHeight; y++) {
            ...
            const finalX = point.x + Math.sin((point.z + Date.now() / 1000) * 0.5) * influence * 30;
            const finalY = point.y + pulse + Math.cos((point.z + Date.now() / 1000) * 0.5) * influence * 30;
            
            // 绘制点
            ctx.beginPath();
            ctx.arc(finalX, finalY, 2 + influence * 3, 0, Math.PI * 2);
            ctx.fillStyle = `rgba(0, ${200 + Math.sin(Date.now() / 1000) * 55}, 255, ${0.3 + influence * 0.7})`;
            ctx.fill();
            
            // 绘制连接线
            if (x < gridWidth - 1) {
              const nextPoint = grid[x + 1][y];
              const nextX = nextPoint.x + Math.sin((nextPoint.z + Date.now() / 1000) * 0.5) * influence * 30;
              const nextY = nextPoint.y + Math.sin(nextPoint.pulse) * 5 + 
                           Math.cos((nextPoint.z + Date.now() / 1000) * 0.5) * influence * 30;
    ...
    function createParticles() {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        // 动画
        particle.animation = particle.animate([
